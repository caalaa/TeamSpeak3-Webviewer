            ...
            switch ($_GET['lang']) {
                case 'de':
                    ...
                    echo(replaceValues( s_root . "html/welcome/welcome.html" ,
                                        $values ,
                                        s_root . "html/welcome/de.i18n.xml" ));
                    break;
                case 'en':
                    ...
                    echo(replaceValues( s_root . "html/welcome/welcome.html" ,
                                        $values ,
                                        s_root . "html/welcome/en.i18n.xml" ));
                    break;
            }
        }
    } else
...
{
    ts3_check( $query->login( $config['username'] , $config['password'] ) ,
                              'login' );
}
...
function render_client( $clientinfo , $servergrouplist , $channelgrouplist )
{
    ...
    foreach ( get_servergroup_images( $clientinfo , $servergrouplist ,
                                      $config['use_serverimages'] ,
                                      $config['servergrp_images'] ) as $image )
    {
        if ( $image == 0 ) continue;
    ...
    $channelgroup_icon = get_channelgroup_image( $clientinfo ,
                                                 $channelgrouplist ,
                                                 $config['use_serverimages'] ,
                                                 $config['channelgrp_images'] );
